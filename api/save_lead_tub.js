...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    ...
    if (certUrl && certUrl.startsWith("https://cert.trustedform.com/")) {
      ...
      // 🛠 Supabase insert
      const { error } = await supabase.from("leads_tub").insert([payload]);
      if (error) {
      ...
      const leadProsperPayload = {
        lp_campaign_id: "28858",
        lp_supplier_id: "86302",
        lp_key: process.env.LEADPROSPER_API_KEY_TUB,
        lp_action: "",
        ...
        existing_tub: payload.existing_tub,
